knn_minus1_break <- function(train,test,class,k,pnrm){
  library(grid)
  library(lattice)
  library(modeltools)
  library(stats4)
  ...
  if (!identical(train,test)){
    ...
    aux<-as.numeric(rownames(as.data.frame(sort(dist_matrix_test[,i]))))[1:k]
    aux_closers<-aux[1:k]
  ...
  if (identical(train,test)){
    for(i in 1:ncol(dist_matrix_test)){
      #aux_closers<-as.numeric(rownames(as.data.frame(sort(dist_matrix[,i]))))[2:(k+1)]
      aux<-as.numeric(rownames(as.data.frame(sort(dist_matrix_test[,i]))))[2:(k+1)]
      aux_closers<-aux[1:k]
      voting<-data.frame(sort(summary(class[aux_closers]),decreasing = TRUE))
      ties<-as.numeric(as.list(summary(voting[,1]==voting[1,1]))$'TRUE')
      if (is.null(ties)){
