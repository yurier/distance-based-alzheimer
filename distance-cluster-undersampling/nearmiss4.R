nearmiss4 <- function(data,class,degree,pnorm){
  library(grid)
  library(lattice)
  library(modeltools)
  library(stats4)
      ...
      if (all(degree!=0,degree_number!=0)){
        while (length(aux_selecta)<degree_number){
          ...
          for (s in colnames(dist_matrix)){
            aux<-dist_matrix[order(dist_matrix[,s],decreasing = TRUE)[1:2],s]
            aux_matrix<-cbind(aux_matrix,c(as.numeric(names(aux[1])),mean(aux[1])))}
          selecta<-as.vector(aux_matrix[1,as.vector(order(aux_matrix[nrow(aux_matrix),],decreasing = TRUE))])
