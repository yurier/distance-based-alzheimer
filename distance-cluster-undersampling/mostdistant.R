mostdistant <- function(data,class,degree,pnorm){
  library(grid)
  library(lattice)
  library(modeltools)
  library(stats4)
  ...
  for (j in levels(class)[!c(levels(class) %in% minor_class_name)]){
    ...
    if (all(nrow(dist_matrix)<2,ncol(dist_matrix)>2,degree_number!=0)){
      ...
      selecta<-dist_matrix[order(dist_matrix,decreasing = TRUE)]
      ...
      if (all(degree!=0,degree_number!=0)){
        while (length(aux_selecta)<degree_number){
          ...
          for (s in colnames(dist_matrix)){
            aux<-dist_matrix[order(dist_matrix[,s],decreasing = FALSE)[1:ifelse(nrow(dist_matrix)<3,nrow(dist_matrix),3)],s]
            aux_matrix<-cbind(aux_matrix,c(as.numeric(s),mean(aux)))}
          colnames(aux_matrix)<-as.character(aux_matrix[1,])
          selecta<-as.vector(aux_matrix[1,as.vector(order(aux_matrix[2,],decreasing = TRUE))])
