SMOTE_border1 <- function(data,class,degree,pnorm,k){
 ...
 for (j in levels(class)[!c(levels(class) %in% major_class_name)]){
    ...
    danger_dista<-dist2(x=data[class==j,],y=data[as.character(danger),],method="minkowski",p=pnrm)
    danger_dista<-as.data.frame(danger_dista)
    colnames(danger_dista)<-as.character(danger)
    rownames(danger_dista)<-rownames(data[class==j,])
