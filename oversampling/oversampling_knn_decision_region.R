...
D<-ggplot()+
  theme_bw()+
  geom_point(data=dataf,aes(x=x, y=y, size=probMCI),fill = "dark grey", colour = "black", alpha = 1/9)+scale_size(range=c(-1, 3))+
  geom_point(aes(x=x, y=y, color=class), size=2,data=data.frame(x=x[class=='MCI',1], y=x[class=='MCI',2], class=class),alpha = 1)+
...
cairo_ps(width=12,height = 3,file='oversampling_smote_knn_plot.eps')
