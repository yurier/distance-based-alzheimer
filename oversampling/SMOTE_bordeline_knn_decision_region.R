...
{CN<-ex2[which(ex2$DX=="NL"),]
  ...
  data<-SMOTE(x,class,1,2,5)
  x<-rbind(x,data[[1]])
  class<-as.factor(c(class,data[[2]]))
  x<-Min_max_nrmlztn(x)
  ...
  C<-ggplot()+
    ...
    theme(legend.text = element_text(size = 15),
          ...
          panel.border = element_blank(),
          panel.background = element_blank())
  dataf$probability<-dataf$probMCI
  D<-ggplot()+
    theme_bw()+
    geom_point(data=dataf,aes(x=x, y=y, size=probability),fill = "dark grey", colour = "black", alpha = 1/9)+scale_size(range=c(-1, 3))+
    geom_point(aes(x=x, y=y, color=class), size=1.4,data=data.frame(x=x[class=='MCI',1], y=x[class=='MCI',2], class=class),alpha = 1)+
  ...
  #cairo_ps(width=13.5,height = 3,file='undersampling_nearmiss1_knn_plot.eps')
  cairo_ps(width=13.5,height = 3,file='oversampling_smote_knn_plot.eps')
...
x<-Min_max_nrmlztn(x)
class<-c(rep("CN",nrow(CN)),rep("MCI",nrow(MCI)),rep("AD",nrow(AD)))
data<-SMOTE_border1(x,class,1,2,k=5)
x<-rbind(x,data[[1]])
class<-as.factor(c(class,data[[2]]))
...
A<-ggplot()+
  ...
  geom_contour(data=dataf,aes(x=x, y=y, z=probCN1), bins=1, colour = "black", alpha = 1,size=1)+
  geom_point(data=dataf,aes(x=x, y=y, size=probCN),fill = "dark grey", colour = "black", alpha = 1/9)+
  scale_size(range=c(-1, 3))+geom_point(data=data.frame(x=x[class=='CN',1], y=x[class=='CN',2], class=class[class=='CN']),aes(x=x, y=y, color=class), size=1.4,alpha = 1)+
  scale_colour_manual(values=c( "#56B4E9"))+
  labs(x='ABETA',y='PTAU')+
  theme(legend.text = element_text(size = 15),
        axis.text=element_text(size=15),
...
B<-ggplot()+
  ...
  geom_contour(data=dataf,aes(x=x, y=y, z=probMCI1), bins=1, colour = "black", alpha = 1,size=1)+
  geom_point(data=dataf,aes(x=x, y=y, size=probMCI),fill = "dark grey", colour = "black", alpha = 1/9)+
  scale_size(range=c(-1, 3))+geom_point(aes(x=x, y=y, color=class), size=1.4,data=data.frame(x=x[class=='MCI',1], y=x[class=='MCI',2], class=class[class=='MCI']),alpha = 1)+
  scale_colour_manual(values=c( "#E69F00"))+
  labs(x='ABETA',y='')+
  theme(legend.text = element_text(size = 15),
        axis.text=element_text(size=15),
...
C<-ggplot()+
  ...
  geom_contour(data=dataf,aes(x=x, y=y, z=probAD1), bins=1, colour = "black", alpha = 1,size=1)+
  geom_point(data=dataf,aes(x=x, y=y, size=probAD),fill = "dark grey", colour = "black", alpha = 1/9)+
  scale_size(range=c(-1, 3))+geom_point(aes(x=x, y=y, color=class), size=1.4,data=data.frame(x=x[class=='AD',1], y=x[class=='AD',2], class=class[class=='AD']),alpha = 1)+
  scale_colour_manual(values=c( "#999999"))+
  labs(x='ABETA',y='')+
  theme(legend.text = element_text(size = 15),
        axis.text=element_text(size=15),
...
D<-ggplot()+
  theme_bw()+
  geom_point(data=dataf,aes(x=x, y=y, size=probability),fill = "dark grey", colour = "black", alpha = 1/9)+scale_size(range=c(-1, 3))+
  geom_point(aes(x=x, y=y, color=class), size=1.4,data=data.frame(x=x[,1], y=x[,2], class=class),alpha = 1)+
  scale_colour_manual(values=c( "#999999", "#56B4E9","#E69F00"))+
  labs(x='ABETA',y='')+
  theme(legend.text = element_text(size = 15),
        axis.text=element_text(size=15),
...
#cairo_ps(width=12,height = 2.5,file='ex5_plus.eps')
#cairo_ps(width=12,height = 2.5,file='ex5_random.eps')
cairo_ps(width=13.5,height = 3,file='smote_bordeline1_knn_plot.eps')
...
x<-Min_max_nrmlztn(x)
class<-c(rep("CN",nrow(CN)),rep("MCI",nrow(MCI)),rep("AD",nrow(AD)))
data<-SMOTE_border2(x,class,1,2,k=5)
x<-rbind(x,data[[1]])
class<-as.factor(c(class,data[[2]]))
row.names(x)<-1:nrow(x)
...
#cairo_ps(width=12,height = 2.5,file='ex5_plus.eps')
#cairo_ps(width=12,height = 2.5,file='ex5_random.eps')
cairo_ps(width=13.5,height = 3,file='smote_bordeline2_knn_plot.eps')
...
{CN<-ex2[which(ex2$DX=="NL"),]
  ...
  x<-Min_max_nrmlztn(x)
  class<-c(rep("CN",nrow(CN)),rep("MCI",nrow(MCI)),rep("AD",nrow(AD)))
  data<-SMOTE_border3(x,class,1,2,k=5)
  x<-rbind(x,data[[1]])
  class<-as.factor(c(class,data[[2]]))
  row.names(x)<-1:nrow(x)
  ...
  #cairo_ps(width=12,height = 2.5,file='ex5_plus.eps')
  #cairo_ps(width=12,height = 2.5,file='ex5_random.eps')
cairo_ps(width=13.5,height = 3,file='smote_bordeline3_knn_plot.eps')
