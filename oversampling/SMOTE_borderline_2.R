SMOTE_border2 <- function(data,class,degree,pnorm,k){
  ...
  for (j in levels(class)[!c(levels(class) %in% major_class_name)]){
    ...
    for (i in index){
      ...
      danger_dista<-dist2(x=data[class==major_class_name,],y=data[as.character(danger),],method="minkowski",p=pnrm)
      danger_dista<-as.data.frame(danger_dista)
      colnames(danger_dista)<-as.character(danger)
      rownames(danger_dista)<-rownames(data[class==major_class_name,])
    ...
    while (nrow(synth_aux)<=balance){
      for (i in index){
        ...
        knear<-knear[complete.cases(knear),]
        #rand<-runif(s,0,1)
