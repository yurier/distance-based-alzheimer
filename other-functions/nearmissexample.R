...
for (i in perc){
  for (j in c(1:CV)){
    ...
    if (i != 0){
      classM<-data.frame(DX='MCI',nearmiss(classD[,-1],classD[,1],classM[,-1],classM[,1],floor((1-i)*(nrow(classM))+(i)*(nrow(classD))),6))
      classN<-data.frame(DX='CN',nearmiss(classD[,-1],classD[,1],classN[,-1],classN[,1],floor((1-i)*(nrow(classN))+(i)*(nrow(classD))),6))}
  ...
  cairo_ps(width=5,height = 5 ,file=sprintf('conf_%gnear.eps',10*i))
...
cairo_ps(width=6,height = 3.5 ,file='nearmissexample.eps')
ggplot()+theme_bw()+
