...
for (i in perc){
  for (j in c(1:CV)){
    ...
    if (i != 0){
      classD<-rbind(classD,data.frame(DX='AD',SMOTE_border(classD[,2:ncol(classD)],classD[,1],classM[,2:ncol(classM)],classM[,1],3,floor(i*(nrow(classM)-nrow(classD))),5)))
      classN<-rbind(classN,data.frame(DX='CN',SMOTE_border(classN[,2:ncol(classN)],classN[,1],classM[,2:ncol(classM)],classM[,1],3,floor(i*(nrow(classM)-nrow(classN))),5)))
    }
  ...
  print(nrow(classM))
  print(nrow(classN))
  cairo_ps(width=5,height = 5 ,file=sprintf('conf_%gBs.eps',10*i))
...
cairo_ps(width=6,height = 3.5 ,file='smoteborderexample.eps')
ggplot()+theme_bw()+
  ...
  labs(x='Oversampling percentage',y='Correct classified average')
dev.off()
