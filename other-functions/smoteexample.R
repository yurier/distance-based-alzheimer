...
PLASMA<-subset(PLASMA, select=c("DX","CDRSB","ADAS11","FDG"))
...
for (i in perc){
  for (j in c(1:CV)){
    ...
    if (i != 0){
    classD<-rbind(classD,data.frame(DX='AD',SMOTE(classD[,-1],floor(i*(nrow(classM)-nrow(classD))),5)))
    classN<-rbind(classN,data.frame(DX='CN',SMOTE(classN[,-1],floor(i*(nrow(classM)-nrow(classN))),5)))
    }
  ...
  print(nrow(classM))
  print(nrow(classN))
  cairo_ps(width=5,height = 5 ,file=sprintf('conf_%gs.eps',10*i))
...
cairo_ps(width=6,height = 3.5 ,file='smoteexample.eps')
ggplot()+theme_bw()+
