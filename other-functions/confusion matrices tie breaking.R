  ...
  set.seed(1)
  ex3_norm<-data.frame(DXCHANGE=ex3[,1],Min_max_nrmlztn(ex3[,2:3]))
  row.names(ex3_norm)<-NULL
  CN<-ex3_norm[which(ex3_norm$DXCHANGE==1),]
  MCI<-ex3_norm[which(ex3_norm$DXCHANGE==2),]
  AD<-ex3_norm[which(ex3_norm$DXCHANGE==3),]
